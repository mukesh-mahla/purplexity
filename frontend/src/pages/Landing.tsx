...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    const syncUser = async () => {
      ...
      await axios.post(`${import.meta.env.VITE_PUBLIC_BACKEND_URL}/create-user`, {}, { headers: { Authorization: `Bearer ${token}` } });
    };
    syncUser();
  }, [isSignedIn, getToken]);
  ...
  const mutation = useMutation({
    mutationFn: async (query: string) => {
      const token = await getToken();
      return axios.post(`${import.meta.env.VITE_PUBLIC_BACKEND_URL}/api/create-conversation`, { query }, { headers: { Authorization: `Bearer ${token}` } });
    },
  });
